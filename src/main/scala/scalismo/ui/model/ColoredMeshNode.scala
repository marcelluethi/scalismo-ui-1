...
class ColorMeshNode(override val parent: ColorMeshesNode, override val source: VertexColorMesh3D, initialName: String)
  ...
  override def transform(untransformed: VertexColorMesh3D, transformation: PointTransformation): VertexColorMesh3D = {
    val trafo = new scalismo.faces.render.Transform3D() {
      override def apply(x: Point[_3D]): Point[_3D] = transformation(x)

      override def apply(v: geometry.Vector[_3D]): geometry.Vector[_3D] = transformation(v.toPoint).toVector
    }
    untransformed.transform(trafo)
  }
  ...
  override def save(file: File): Try[Unit] = scalismo.faces.io.MeshIO.write(transformedSource, file)
