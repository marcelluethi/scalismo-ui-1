...
class ScalarRangeProperty(initialValue: ScalarRange, val colorMapping : ColorMapping = ScalarRangeProperty.DefaultColorMap) extends NodeProperty[ScalarRange](initialValue) {
  def this() = this(
